...
export default function Building2({ position, scale, rotation }) {
    ...
    const { materials, animations, scene } = useGLTF("/3D_models/b2/scene.gltf");
    const { actions } = useAnimations(animations, modelRef);

    const { opacity } = useSpring({
        opacity: !s_isFocus || s_focusTargetMain === "Building2" ? 1 : 0.5,
    ...
    const handleClick = (e) => {
        ...
        if (s_isFocus) {
            ...
            dispatch(change_s_camPosNTarget("Building2"));
        }
    };
    ...
    useEffect(() => {
        if (s_shouldUnMount) {
            dispatch(change_s_view1Component(prev => prev.filter(x => x.name !== "Building2")));
        }
    }, [s_shouldUnMount]);
    ...
    return (
        <group ref={modelRef} position={position} scale={scale} rotation={rotation}>
            ...
            <Gltf src="/3D_models/b2/scene.gltf" position={[0, 0, 0]} onClick={handleClick}/>
        </group>
    );
}
